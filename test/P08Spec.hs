...
puzzle = ["gea agfcbe egcbfd aecb cegbf cafgde fgeba ea gabdf begfdca | bcfeg ea fbdga fbgcdae"
...
spec = do
    describe "sum1478" $ do
        it "counts occurences of digits with 2,4,3 and 7 segments in output values" $ do
            sum1478 sample `shouldBe` 26
        it "should pass the puzzle" $ do
            sum1478 puzzle `shouldBe` 445

    describe "swapDigitModel" $ do
        it "swap segments from a standard digit pattern with segments from a permutation" $ do
            swapDigitModel "cf" "gfedcba" `shouldBe` "eb"

    describe "permutedDigit" $ do
        it "gives a digit from swapping a digit with a permutation" $ do
            permutedDigit "gfedcba" ("cf",1) `shouldBe` ("eb",1)

    describe "permutedDigits" $ do
        it "swaps all the digits with a permutation" $ do
            permutedDigits "ebfcdag" `shouldBe`[("ebfdag",0),("fa",1),("efcdg",2),("efcag",3),("bfca",4),("ebcag",5),("ebcdag",6),("efa",7),("ebfcdag",8),("ebfcag",9)]

    describe "isPermutedWith" $ do
        it "tells if an entry represents digits according to a permutation" $ do
            let e = map sort $ words "ecgfd ebfacg bcage dcefg"
            e `isPermutedWith` "ebfcdag" `shouldBe` True

    describe "permutedWith" $ do
        it "tells digits according to a permutation" $ do
            let e = map sort $ words "ecgfd ebfacg bcage dcefg"
            e `permuteWith` "ebfcdag" `shouldBe` [Just 2,Just 9,Just 5,Just 2]

    describe "value" $ do
        it "tells the value from an entry according to a permutation" $ do
            let e = map sort $ words "ecgfd ebfacg bcage dcefg"
            value e "ebfcdag" `shouldBe` 2952

    describe "findValue" $ do
        it "tells the value from an entry by trying permutations" $ do
            let e = "af cegbfa bfca egdabf aef bgeac cfaeg caebdg gdecf gcfebda | ecgfd ebfacg bcage dcefg"
            findValue e `shouldBe` 2952

    describe "all of this" $ do
        it "should pass the sample" $ do
            solve sample `shouldBe` 61229

        it "should pass the puzzle" $ do
            solve puzzle `shouldBe` 1043101
